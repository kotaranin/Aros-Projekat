...
void *dedaMrazFunkcija(void *arg) {
  ...
  while (1) {
    ...
    if (irvasi == BROJ_IRVASA) {
      ...
      for (int r = 0; r < BROJ_IRVASA; r++) sem_post(irvasSemafor);
      ...
      if (patuljci > 0)
        printf("Deda Mraz pomaze svim patuljcima koji su trazili pomoc\n");
...
void kreiranjeNiti() {
  ...
  for (int i = 0; i < BROJ_IRVASA; i++)
    pthread_create(&irvasiNiti[i], NULL, irvasiFunkcija, (void *)&irvasiNiz[i]);
  for (int i = 0; i < BROJ_PATULJAKA; i++)
    pthread_create(&patuljciNiti[i], NULL, patuljakFunkcija,
                   (void *)&patuljciNiz[i]);
}

void zatvaranjeSemafora() {
  sem_close(dedaMraz);
